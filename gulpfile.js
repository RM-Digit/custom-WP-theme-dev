...
gulp.task( 'minifycss', function() {
  return gulp.src( [paths.css + '/theme.css',paths.css + '/admin.css',paths.css + '/salesfusion.css'] )
  .pipe( sourcemaps.init( { loadMaps: true } ) )
...
gulp.task( 'scripts', function() {
    var scripts = [
        ...
        paths.dev + '/js/jquery.validate.js',
        paths.dev + '/js/jquery.caret.js',
        paths.dev + '/js/jquery.mask.js',
        paths.dev + '/js/jquery.mobilePhoneNumber.js',
        paths.dev + '/js/additional-methods.js',
        paths.dev + '/js/quest-g-recaptcha.js',
  ...
  gulp.src( scripts )
    ...
    .pipe( gulp.dest( paths.js ) );
  gulp.src( [ paths.dev + '/js/salesfusion.js',] )
      .pipe( uglify() )
    .pipe( concat( 'salesfusion.min.js' ) )
    .pipe( gulp.dest( paths.js ) );
  gulp.src( [ paths.dev + '/js/head-javascript.js',] )
      .pipe( uglify() )
    .pipe( concat( 'head-js.min.js' ) )
    .pipe( gulp.dest( paths.js ) );
});
...
// Copy all Font Awesome Fonts
    gulp.src([paths.node + 'font-awesome/fonts/**/*.{ttf,woff,woff2,eot,svg}', paths.dev +'/fonts/**/*.{ttf,woff,woff2,eot,svg}'])
        .pipe( gulp.dest( './fonts' ) );
// Copy admin js
    gulp.src( paths.dev + '/js/quest-init-admin.js')
        .pipe( gulp.dest( './js' ) );
...
// Copy Popper JS files
    ...
    gulp.src( paths.node + 'popper.js/dist/umd/popper.min.js.map' )
        .pipe( gulp.dest( paths.js + paths.vendor ) );
    gulp.src( paths.node + 'popper.js/dist/umd/popper.js' )
        .pipe( gulp.dest( paths.js + paths.vendor ) );

    gulp.src( paths.node + 'slick-carousel/slick/**/*' )
        .pipe( gulp.dest( 'js/slick' ) );
    gulp.src( paths.dev + '/js/jquery-f-select/fSelect.js' )
        .pipe( gulp.dest( 'js/jquery-f-select' ) );

    gulp.src( paths.dev + '/js/tiny-mce/**/*' )
        .pipe( gulp.dest( 'js/tiny-mce' ) );
...
gulp.task( 'build', function( callback ) {
    gulpSequence( 'copy-assets', 'scripts' , 'styles' )( callback );
});
