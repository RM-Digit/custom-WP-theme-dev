        ...
        <div class="bounder">
            <div class="container">
                <h1 class="search-header-title">
                    <?php _e('Archived Strategic Advisor Newsletter', 'quest');?>
                </h1>
            </div>
        </div>
    </div>
        ...
        <?php if ( $newsletters->have_posts() ) :?>
            <div class="row">
                <div class="desktop-drop-down drop-down offset-md-10 col-md-2 pl-0">
                    <form id="press-release-sort" method="get" action="<?php echo get_post_type_archive_link(QUEST_POST_TYPE_RESOURCE_NEWSLETTER);?>">
                        <?php quest_sortable();?>
                    </form>
                </div>
            </div>
