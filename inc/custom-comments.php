...
if ( ! function_exists( 'understrap_bootstrap_comment_form' ) ) {
	...
	function understrap_bootstrap_comment_form( $args ) {
	    ...
	    <label for="comment">' . _x( 'Comment', 'noun', 'understrap' ) . ( ' <span class="required" style="color: red">*</span>' ) . '</label>
...
add_filter( 'comment_form_defaults', 'understrap_bootstrap_must_log_in_form' );
if ( ! function_exists( 'understrap_bootstrap_must_log_in_form' ) ) {

    function understrap_bootstrap_must_log_in_form( $args ) {
        $args['must_log_in'] = sprintf(
            __( '<div><p class="must-log-in pb-2 pt-2">
                 <a href="%s">Logged in</a> to post a comment.</p>
                 <a class="btn btn-secondary text-white"> Learn More</a>
                 </div>'
            ),
            wp_registration_url(),
...
} // endif function_exists( 'understrap_bootstrap_must_log_in_form' )
